...
class Gun:
    def __init__(self, type: str) -> None:
        ...
        self.bullet_size = self.bullet_data["size"]
        self.bullet_speed = self.bullet_data["speed"]
        self.bullet_lifetime = self.bullet_data["lifetime"]
        self.bullet_pierce = self.bullet_data["pierce"]

        self.bullets: list[Bullet] = []

        self.rect = pygame.rect.Rect((0, 0), (self.bullet_size, self.bullet_size))
    ...
    def shoot(self, target: tuple[int, int], start_pos: tuple[int, int]) -> None:
        ...
        for i in range(num_bullets):
            if num_bullets == 1:
                ...
                angle_offset = (i - (num_bullets - 1) / 2) * spread_angle / max(1, (num_bullets - 1))
